...
{
    auto first = m_genom.begin() + startCommand;
    auto last = m_genom.begin() + startCommand + commandLength;
...
int Genom::parseExpression(const size_t startCommand, long long& energy, size_t& next)
{
    ...
    int lhs, rhs;
    int result = 0;
    next = startCommand + CommandLength::OPERATION;
    maths nextCommand = static_cast<maths>(getNextOperation(startCommand));
    switch (nextCommand) {
    case NUMBER_CONST:
