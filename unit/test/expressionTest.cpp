...
void
testExpression(const std::vector<char>& gen,
...
TEST(ParseExpression, Const)
{
  ...
  testExpression(
    gen, CommandLength::MATH + CommandLength::NUMBER, START_ENERGY - 1, 1);
}
...
TEST(ParseExpression, Answer)
{
  ...
  testExpression(
    gen, finalPos, START_ENERGY - 1, static_cast<size_t>(answer::Empty));
